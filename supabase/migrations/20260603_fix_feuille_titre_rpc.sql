-- Correction : feuille_titre et obj_global_feuille_titre utilisaient noeud.nom
-- (nom de la catégorie parente) au lieu de feuille_entrainement.titre
-- (titre propre de la feuille). Le type de retour ne change pas → pas de DROP.
...
AS $$
  ...
  SELECT
    ...
    objr.feuille_id,
    fe.titre::text           AS feuille_titre,          -- corrigé : fe.titre au lieu de noeud.nom
    objr.ref_exercice,
    objr.note::text,
    objg.feuille_id          AS obj_global_feuille_id,
    fe_g.titre::text         AS obj_global_feuille_titre, -- corrigé : fe_g.titre au lieu de ng.nom
